import{Logger as k,removeEndingSlash as h,entries as m,removeLeadingSlash as g,isLinkHttp as P,isLinkAbsolute as b,deepAssign as R,fromEntries as p,isPlainObject as w,isArray as L,keys as z,isFunction as S,getModulePath as _,getLocaleConfig as C,addViteSsrNoExternal as E}from"@vuepress/helper";import{fs as s,path as f,colors as j,getDirname as A}from"vuepress/utils";import{createPage as D}from"vuepress/core";const M=async a=>{a.pages.every(({path:e})=>e!=="/")&&a.pages.push(await D(a,{path:"/",frontmatter:{title:"Home"},content:`Redirecting to the correct locale...
`}))},v="@vuepress/plugin-redirect",d=new k(v),F=({config:a,defaultBehavior:e,defaultLocale:c,localeFallback:n},t,i)=>`<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
`,B=async({dir:a,options:e,pages:c},n)=>{const t=c.filter(({pathLocale:o})=>o==="/").map(({path:o})=>o),i={};c.filter(({pathLocale:o})=>o!=="/").forEach(({path:o,pathLocale:l})=>{const r=o.replace(l,"/").replace(/\/$/,"/index.html");t.includes(r)||(i[r]??=[]).push(l)});const{succeed:$}=d.load("Generating locale redirect files");await Promise.all(m(i).map(async([o,l])=>{const r=a.dest(g(o));s.existsSync(r)||(await s.ensureDir(f.dirname(r)),await s.writeFile(r,F(n,l,e.base)))})),$()},T=a=>`<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="robots" content="noindex">
